...
try{
 ...
 await page.addInitScript(()=>{
   window.labels=[];window.online=true;window.hang=false;window.resizeCalls=0;
   ...
   window.__TAURI_INTERNALS__={transformCallback(){return 1;},unregisterCallback(){},metadata:{currentWindow:{label:'dino-hud'},currentWebview:{label:'dino-hud'}},async invoke(cmd){
    ...
    if(cmd==='resize_hud'){window.resizeCalls++;return null;} if(cmd==='era_live_state'&&window.hang)return new Promise(()=>{}); if(cmd==='era_live_state')return {status:window.online?'online':'error',receivedAt:Date.now(),data:{player:{healthPercent:75,hungerPercent:53,thirstPercent:71,staminaPercent:14,growthPercent:29,prime:{available:true,conditions:Array.from({length:10},(_,i)=>({id:i+1,complete:i<3}))}}}};
 ...
 const count=await page.evaluate(()=>window.labels.length);
 await page.waitForTimeout(2200);
 assert.equal(await page.evaluate(()=>window.labels.length),count,'Dữ liệu không đổi không vẽ lại canvas');
 assert.equal(await page.locator('#resize-handle').evaluate(e=>getComputedStyle(e).pointerEvents),'none');
 await page.locator('#resize-handle').dispatchEvent('pointerdown',{button:0});
 assert.equal(await page.evaluate(()=>window.resizeCalls),0,'Không resize khi tay nắm ẩn');
 await page.evaluate(()=>{window.hang=true;window.labels=[];});
 await page.waitForFunction(()=>document.body.textContent.includes('Chờ dữ liệu mới'));
 assert.deepEqual(errors,[]);
console.log('PASS: cached HUD painting, invisible resize handle disabled, stale expiry even with hung snapshot RPC.');
