...
try{
 const page=await browser.newPage({viewport:{width:280,height:258}}),errors=[];
 ...
 await page.addInitScript(()=>{
   ...
   window.__TAURI_INTERNALS__={transformCallback(){return 1;},unregisterCallback(){},metadata:{currentWindow:{label:'dino-hud'},currentWebview:{label:'dino-hud'}},async invoke(cmd){
    if(cmd==='get_settings')return {language:'vi',position_source:'era',minimap:{opacity:1},dino_hud:{width_px:280}};
 ...
 await page.waitForFunction(()=>window.labels.includes('Nhiệm vụ Prime 3/10'));
 ...
 assert.equal(await page.locator('canvas').evaluate(c=>parseFloat(c.style.height)),258);
 await mkdir('docs/qa-output',{recursive:true});await page.screenshot({path:'docs/qa-output/prime-hud-vi.png'});
 await page.evaluate(()=>{window.online=false;window.labels=[];});
 await page.waitForFunction(()=>window.labels.includes('Nhiệm vụ Prime —/10'));
 assert.deepEqual(errors,[]);
 console.log('PASS: Vietnamese HUD, 10 Prime conditions, stale reset, matching 258px geometry, smooth rAF zoom, one debounced save. No live game mutation.');
