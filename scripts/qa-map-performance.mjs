// Đo nhịp khung hình khi kéo và zoom với dữ liệu Atlas và IPC giả lập.
...
try {
  ...
  await page.goto('http://127.0.0.1:1420/#map');
  ...
  const label = page.locator('.leaflet-tooltip').filter({hasText:'Tyrannosaurus'});
  await page.evaluate(() => {
    window.frameGaps=[];window.measuring=true;let last=performance.now();
    function sample(now){if(!window.measuring)return;window.frameGaps.push(now-last);last=now;requestAnimationFrame(sample);}
    requestAnimationFrame(sample);
  });
  for (let round=0;round<4;round++) {
    await page.mouse.move(700,430);
    await page.mouse.wheel(0,round%2 ? 240 : -240);
    await page.waitForTimeout(320);
    await page.mouse.move(740,430);
    await page.mouse.down();
    await page.mouse.move(1020,530,{steps:25});
    await page.mouse.move(740,430,{steps:25});
    await page.mouse.up();
    await page.waitForTimeout(250);
  }
  const timing=await page.evaluate(()=>{window.measuring=false;const a=window.frameGaps.slice(1).sort((a,b)=>a-b);return {samples:a.length,averageFps:1000/(a.reduce((x,y)=>x+y,0)/a.length),p95Ms:a[Math.floor(a.length*.95)],over50ms:a.filter(x=>x>50).length};});
  assert.ok(timing.samples>30);
  assert.deepEqual(errors,[]);
  await page.screenshot({path:'docs/qa-output/hey-guys-map-performance.png'});
  console.log(JSON.stringify(timing));
  console.log('PASS: Canvas layers, repeated wheel zoom and drag, no browser errors. Browser RAF is a responsiveness proxy, not an in-game FPS guarantee.');
