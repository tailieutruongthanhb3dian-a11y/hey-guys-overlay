...
render(canvas,state);
const text=[];const context=canvas.getContext('2d');const fillText=context.fillText.bind(context);context.fillText=(value,...args)=>{text.push(value);fillText(value,...args);};
state.panelH=92;state.dino={percentages:true,hp:{current:75,max:100},hunger:{current:null,max:100},thirst:{current:30,max:100},stamina:{current:60,max:100},growthPct:85};
render(canvas,state);
if(!text.includes('75%')||text.includes('75/100')||!text.includes('—'))throw new Error('ERA HUD must show percentages and missing values accurately');
...
});assert.equal(result,true);await page.screenshot({path:'docs/qa-output/minimap-navigation.png'});console.log('PASS: navigation arrow, ERA percentage labels, missing data shown as dash');
