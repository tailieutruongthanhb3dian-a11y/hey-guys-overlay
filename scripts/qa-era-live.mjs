...
try {
  ...
  await page.addInitScript(() => {
    ...
    window.__TAURI_INTERNALS__.invoke = async (cmd,args) => {
      if(cmd === 'era_live_state') return {status:'online',receivedAt:Date.now(),data:{player:{class:'Raptor',name:'Test player',healthPercent:82.5,staminaPercent:41,growthPercent:75,hungerPercent:60,thirstPercent:45,exactVitals:{health:825,maxHealth:1000},location:{x:0,y:0,z:12300}}}};
      return original(cmd,args);
    };
  });
  await page.clock.install();
  await page.goto('http://127.0.0.1:1420/#dino');
  await page.getByRole('heading',{name:'Raptor',exact:true}).waitFor();
  assert.equal(await page.locator('.stats').innerText().then(t=>t.includes('82.5%')),true);
  assert.match(await page.locator('.stats').innerText(),/825 \/ 1\.000/);
  assert.match(await page.locator('.stats').innerText(),/60.0%/);
  await page.screenshot({path:'docs/qa-output/era-live.png',fullPage:true});
  await page.clock.fastForward(6000);
  assert.equal(await page.locator('.stats').innerText().then(t=>t.includes('82.5%')),false,'Old health must not remain current');
  assert.equal(await page.locator('.stats strong').allTextContents().then(a=>a.every(s=>s==='\u2014')),true);
  assert.deepEqual(errors,[]);
  console.log('PASS: ERA dino default, actual percentages, no stale vitals after 5 seconds. IPC fixture only.');
