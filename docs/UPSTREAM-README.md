...
**Tiếng Việt** · [English](UPSTREAM-README.en.md)
...
![Minimap có chấm bạn bè, thanh chỉ số, nhiệm vụ Prime và thanh Thoại đè lên game — con dino đang mang skin vừa đổi trong app](screenshot-ingame.jpg)

![Bản đồ lớn với tên địa danh và các lớp POI](screenshot-fullmap.png)

![Tab Khủng long của bạn với chỉ số và Prime progress](screenshot-dino.png)

![Tab Garage (Gacha): dino đã gửi kho, xem 3D, growth và chỉ số, nút Restore](screenshot-garage.png)

![Skin editor: 10 vùng màu, xem trước 3D đúng loài đang chơi, áp trực tiếp lên dino](screenshot-skin.png)

![Tab Thoại: nói theo khoảng cách qua IsleVOIP, chọn micro và người trong tầm nghe](screenshot-voip.png)
...
1. Mở trang server trong trình duyệt và đăng nhập Steam ở đó. Bấm **F12**
   ...
   **Storage** (Firefox).
   ...
   ![Mở DevTools và chọn tab Application](guide-dino-1-devtools.png)

2. Chọn **Cookies** → domain của server → bấm cookie tên **`islepilot_player`**
   → copy toàn bộ **Value**.

   ![Copy giá trị cookie islepilot_player](guide-dino-2-copy-cookie.jpg)

3. Trong app: dán vào ô cookie → bấm **Kiểm tra & lưu cookie**.

   ![Nhập link server, dán cookie và lưu trong app](guide-dino-3-paste-app.jpg)
